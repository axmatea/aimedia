...
export interface CinematicHeroProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  accentColor?: string;
  badgeLeft?: { emoji: string; title: string; subtitle: string };
  badgeRight?: { emoji: string; title: string; subtitle: string };
  widgetRows?: Array<{ icon: string; color: string }>;
}
...
export function CinematicHero({
  ...
  accentColor = "#3B82F6",
  badgeLeft = { emoji: "🔥", title: "1 Year Streak", subtitle: "Milestone unlocked" },
  badgeRight = { emoji: "🤝", title: "Sponsor Update", subtitle: "Shared successfully" },
  widgetRows = [
    { icon: "check-circle", color: "from-blue-500/20 to-blue-600/5" },
    { icon: "check", color: "from-emerald-500/20 to-emerald-600/5" },
  ],
  className,
  ...props
}: CinematicHeroProps) {
        ...
        >
          ...
          <div className="relative w-full h-full max-w-7xl mx-auto px-4 lg:px-12 flex flex-col justify-evenly lg:grid lg:grid-cols-3 items-center lg:gap-8 z-10 py-6 lg:py-0">
            ...
            <div className="card-right-text gsap-reveal order-1 lg:order-3 flex justify-center lg:justify-end z-20 w-full">
              <h2 className="text-5xl md:text-[6rem] lg:text-[7rem] font-black uppercase tracking-tighter text-card-silver-matte text-center lg:text-right leading-none">
                {brandName}
              </h2>
            </div>
              ...
              <div className="relative w-full h-full flex items-center justify-center transform scale-[0.65] md:scale-85 lg:scale-100">
                  ...
                  <div className="absolute inset-[7px] bg-[#050914] rounded-[2.5rem] overflow-hidden shadow-[inset_0_0_15px_rgba(0,0,0,1)] text-white z-10">
                    ...
                    <div className="relative w-full h-full pt-12 px-5 pb-8 flex flex-col">
                      <div className="phone-widget flex justify-between items-center mb-8">
                        <div>
                          <span className="text-[10px] text-neutral-400 uppercase tracking-widest font-bold mb-1 block">Nearby</span>
                          <span className="text-xl font-bold tracking-tight text-white">Wi-Fi Map</span>
                        </div>
                      ...
                      <div className="phone-widget relative w-44 h-44 mx-auto flex items-center justify-center mb-8 drop-shadow-[0_15px_25px_rgba(0,0,0,0.8)]">
                        <svg className="absolute inset-0 w-full h-full">
                          ...
                          <circle className="progress-ring" cx="88" cy="88" r="64" fill="none" stroke={accentColor} strokeWidth="12" />
                        </svg>
                      ...
                      <div className="space-y-3">
                        {widgetRows.map((row, i) => (
                          <div key={i} className="phone-widget widget-depth rounded-2xl p-3 flex items-center">
                            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${row.color} flex items-center justify-center mr-3 border border-white/10`}>
                              {row.icon === "wifi" ? (
                                ...
                                <svg viewBox="0 0 24 24" className="w-4 h-4 text-yellow-400" fill="none" stroke="currentColor" strokeWidth="2">
                                  <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>
                                </svg>
                              ) : row.icon === "check-circle" ? (
                                <svg viewBox="0 0 24 24" className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <div className="floating-badge absolute flex top-6 lg:top-12 left-[-15px] lg:left-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-gradient-to-b from-blue-500/20 to-blue-900/10 flex items-center justify-center border border-blue-400/30">
                    <span className="text-base lg:text-xl drop-shadow-lg">{badgeLeft.emoji}</span>
                  </div>
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">{badgeLeft.title}</p>
                    <p className="text-blue-200/50 text-[10px] lg:text-xs font-medium">{badgeLeft.subtitle}</p>
                  </div>
                </div>
                ...
                <div className="floating-badge absolute flex bottom-12 lg:bottom-20 right-[-15px] lg:right-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-gradient-to-b from-indigo-500/20 to-indigo-900/10 flex items-center justify-center border border-indigo-400/30">
                    <span className="text-base lg:text-lg drop-shadow-lg">{badgeRight.emoji}</span>
                  </div>
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">{badgeRight.title}</p>
                    <p className="text-blue-200/50 text-[10px] lg:text-xs font-medium">{badgeRight.subtitle}</p>
                  </div>
                </div>
              </div>
            </div>
