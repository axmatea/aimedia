...
export function Calendar({
  ...
  const defaultComponents = {
    Chevron: ({ className, orientation, ...props }: { className?: string; orientation?: "left" | "right" | "up" | "down" }): React.ReactElement => {
      if (orientation === "left") return <ChevronLeftIcon className={cn(className, "rtl:rotate-180")} {...props} aria-hidden="true" />;
      if (orientation === "right") return <ChevronRightIcon className={cn(className, "rtl:rotate-180")} {...props} aria-hidden="true" />;
